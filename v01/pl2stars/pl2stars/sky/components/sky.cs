...
using pl2.rainbow.stars.coorinates;
...
namespace pl2.rainbow.stars.sky.components
{
    public partial class Sky : UserControl
    {
        ...
        public static string CurStars; //текущее созвездие
        public static bool Coorinate_visible;
        ...
        {
            Pen p = Pens.Snow;
            e.Graphics.DrawEllipse(p, 10, 20, 30, 30);
            Draw( true );
        }
        ...
        private void Draw(bool force)
        {
            ...
            string s;
            double x1 , x2;
            sin_f = (float)Math.Sin( f0 );
            cos_f = (float)Math.Cos( f0 );
            if (!force)
            {
                // clear
                /*
            ...
            sin_w = (float)Math.Sin( w0 );
            cos_w = (float)Math.Cos( w0 );
            if (force)
            {
                for (i = 0 ; i < Catalog.almanac.Length ; ++i)
                {
                    Catalog.almanac[i].DrawStar();
